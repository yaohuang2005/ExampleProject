...
void StringUtil::RTrim(std::string *str) {
    // remove trailing ' ', \f, \n, \r, \t, \v
    str->erase(std::find_if(str->rbegin(), str->rend(), [](int ch) { return std::isspace(ch) == 0; }).base(),
               str->end());
}
...
std::string StringUtil::string_replace(const std::string &str, const std::string &match,
...
bool StringUtil::incensitiveEquals(const std::string &str1, const std::string &str2) {
    return std::equal(str1.begin(), str1.end(), str2.begin(), iequal());
}

std::string StringUtil::concatStringInt(const std::string &a, int b) {
    std::stringstream ss;
    ss << a << b;
...
template<typename T>
std::string StringUtil::to_string(const T &thing) {
    std::stringstream ss;
    ss << thing;
    return ss.str();
}
...
template<>
std::string StringUtil::to_string<std::string>(const std::string &s) {
    return s;
}
